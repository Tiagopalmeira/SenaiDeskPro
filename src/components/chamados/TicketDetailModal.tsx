...
import { obterImagemSolicitacao } from "@/api_requests/solicitacoes";
import {
    ModalOverlay,
    ModalContent,
    ModalHeader,
...
export function TicketDetailModal({ isOpen, onClose, solicitacao, userId, isAdmin, onUpdate }: TicketDetailModalProps) {
    const { status, setStatus, resposta, setResposta, loading, error, success, handleAssinar, handleSalvar } =
    ...
    const updateImagemUrl = (url: string | null) => {
    ...
    useEffect(() => {
        ...
        const frame = requestAnimationFrame(() => {
            ...
            obterImagemSolicitacao(solicitacao.id_solicitacao)
                .then((result) => {
                    if (cancelled) return;
                    updateImagemUrl(result?.dataUrl ?? null);
                })
                .catch(() => {
                    if (!cancelled) {
                        updateImagemUrl(null);
                    }
                })
                .finally(() => {
                    if (!cancelled) {
    ...
    return (
        <ModalOverlay onClick={onClose}>
            <ModalContent onClick={(e) => e.stopPropagation()}>
                ...
                {success && <SuccessMessage>Alterações salvas com sucesso!</SuccessMessage>}

                <DetailSection>
                    <DetailLabel>Status</DetailLabel>
                    <DetailValue>
                        <StatusBadge $status={solicitacao.status}>{solicitacao.statusLabel}</StatusBadge>
                    </DetailValue>
                </DetailSection>
                ...
                <DetailSection>
                    ...
                    <DetailValue>
                        <PriorityBadge $priority={solicitacao.prioridade}>{solicitacao.prioridadeLabel}</PriorityBadge>
                    </DetailValue>
                </DetailSection>
                ...
                <DetailSection>
                    ...
                    <DetailValue>{solicitacao.dataUltimaAtualizacaoFormatada || "Não modificado"}</DetailValue>
                </DetailSection>

                <DetailSection>
                    <DetailLabel>Responsável</DetailLabel>
                    <DetailValue>{solicitacao.nome_responsavel || "Aguardando responsável"}</DetailValue>
                </DetailSection>

                <DetailSection>
                    <DetailLabel>Imagem</DetailLabel>
                    <DetailValue>
                        {imagemLoading && "Carregando imagem..."}
                        {!imagemLoading && imagemUrl && (
                            <img
                                src={imagemUrl}
                                alt={`Imagem do chamado ${solicitacao.id_solicitacao}`}
                                style={{ width: "100%", height: "auto", borderRadius: "8px" }}
                            />
                        )}
                        {!imagemLoading && !imagemUrl && "Nenhuma imagem anexada."}
                    </DetailValue>
                </DetailSection>
                ...
                {podeEditar && (
                    <>
                        <FormGroup>
                            ...
                            <Select value={status} onChange={(e) => setStatus(Number(e.target.value) as 0 | 1 | 2)}>
                ...
                {!isAdmin && (
                    <DetailSection>
                        ...
                        <DetailValue>
                            Você pode visualizar os detalhes da sua solicitação. O setor responsável entrará em contato
                            em breve.
                        </DetailValue>
                    </DetailSection>
                )}
